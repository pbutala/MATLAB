...
CHAROVERWRITE = '~';
if(fARCHIVE)
    CHARIDXARCHIVE = '_varMg';           % ARCHIVE INDEX
else
...
switch fSTATION
    case 1
        ctDirRes = '\\ad\eng\users\p\b\pbutala\My Documents\MATLAB\Research\Code V2\Results\8. OSM All 1-M\';
        ctFileCodeSrc = '\\ad\eng\users\p\b\pbutala\My Documents\MATLAB\Research\Code V2\Scripts\scrOSM_mpAll.m';
    case 2
        ctDirRes = '/home/pbutala/My Documents/MATLAB/Research/Code V2/Results/8. OSM All 1-M/';
        ctFileCodeSrc = '/home/pbutala/My Documents/MATLAB/Research/Code V2/Scripts/scrOSM_mpAll.m';
    case 3
        ctDirRes = 'C:\\Users\\pbutala\\Documents\\MATLAB\\Research\\Code V2\\Results\\8. OSM All 1-M\\';
        ctFileCodeSrc = 'C:\\Users\\pbutala\\Documents\\MATLAB\\Research\\Code V2\\Scripts\\scrOSM_mpAll.m';
    otherwise
        error('Station not defined');
end
ctFileCodeDest = [ctDirRes 'scrOSM_mpAll' CHARIDXARCHIVE '.m'];
ctFileVars = [ctDirRes 'OSM_mpAll' CHARIDXARCHIVE '.mat'];      % file to store workspace
...
for iNt = 1:lenNt
    ...
    for iMS = 1:lenMS
        ...
        %     for iAS = 1:lenAS
        %         AS = rngAS(iAS);
        %         STRAS = sprintf('As %0.2f,',AS);
        %         txa = AS*(2-opf)*rxal/(opf*sqrt(2)); % AS*2*rxal/opf is the diagonal of tx. divide by sqrt(2) to get side length of square tx.
        %         for iES = 1:lenES
        %             ES = rngES(iES);
        %             STRBS = sprintf('Es %0.2f,',ES);
        %             DS = ES + AS/sqrt(2);
        %             txD = DS*sqrt(2)*rxal*(2-opf)/(opf);
        %         opf = rngF(iF);
        %             opfN = opf/opD; % f/#
        %         STRF = sprintf('fcs %0.0fmm,',opf*1e3);
        tM = 1/(MS*Mg0);
        opf = 2/(1+tM);
            ...
            for iMOD = 1:lenMOD
                ...
                switch MODSEL
                    case MODnSM                 % nSM BPS = log2(Nt*M)
                        ...
                        clLgd{iLgd(iNt,iBPS),iNt,iBPS} = sprintf('SMod: \\mu_{s}=%0.2f',MS);
                    case MODeSM                 % eSM BPS = Nt*log2(M)
                        ...
                        clLgd{iLgd(iNt,iBPS),iNt,iBPS} = sprintf('SMux: \\mu_{s}=%0.2f',MS);
                end
