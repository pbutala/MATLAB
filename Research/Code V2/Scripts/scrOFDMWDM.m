...
try
    ...
    for iTsd = 1:LENLEDWID                                                       % LOOP START LED SD
        ...
        switch fSTATION
            ...
            case 1
                ...
                sPSDDIR = [ctMatDir sSPDTYP '\' sprintf('R_%d_%d_%d_G_%d_%d_%d_B_%d_%d_%d',...
                    RMN,RNGLEDWID(iTsd),RSC,GMN,RNGLEDWID(iTsd),GSC,BMN,RNGLEDWID(iTsd),BSC) '\'];
            case 2
        ...
        switch lower(sSPDTYP)
            case 'gaussian'
                Rspd = getSOG(RMN,RNGLEDWID(iTsd),RSC,lambdas);
                Gspd = getSOG(GMN,RNGLEDWID(iTsd),GSC,lambdas);
                Bspd = getSOG(BMN,RNGLEDWID(iTsd),BSC,lambdas);
            case 'lorentzian'
                Rspd = getLorentzian(RMN,RNGLEDWID(iTsd),lambdas);
                Gspd = getLorentzian(GMN,RNGLEDWID(iTsd),lambdas);
                Bspd = getLorentzian(BMN,RNGLEDWID(iTsd),lambdas);
            otherwise
                    ...
                    for iOf = 1:LENOFDMTYPES                                            % LOOP START OFDM types
                        ...
                        while ~LOOPDONE                                                 % LOOP START SNR (dynamically select next SNR)
                            ...
                            if (RNGSNRDB(iSNR,iTsd,iT,iFW,iNSC,iOf) > RNGSNRMAX) || isequal(sum(BER(:,iSNR,iTsd,iT,iFW,iNSC,iOf) < BERTHMIN),NTX)
                                LOOPDONE = true;
                                LOOPCOUNT = (iTsd-1)*LENCCT*LENFWHM*LENMODNSC*LENOFDMTYPES*RNGSNRLOOP +...
