...
rngIllSet = [200 400 600 800]; % varILLSet
...
switch fSTATION
    case 1
        ...
        ctFileCodeSrc = '\\ad\eng\users\p\b\pbutala\My Documents\MATLAB\Research\Code V2\Scripts\scrSVDVLC_OWC.m';
    case 2
        ctDirRes = '/home/pbutala/My Documents/MatlabResults/001. SVDVLC OWC/';
        ctFileCodeSrc = '/home/pbutala/My Documents/MATLAB/Research/Code V2/Scripts/scrSVDVLC_OWC.m';
    case 3
        ctDirRes = 'C:\\Users\\pbutala\\Documents\\MatlabResults\\001. SVDVLC OWC\\';
        ctFileCodeSrc = 'C:\\Users\\pbutala\\Documents\\MATLAB\\Research\\Code V2\\Scripts\\scrSVDVLC_OWC.m';
    otherwise
        error('Station not defined');
end

ctFileCodeDest = [ctDirRes 'scrSVDVLC_OWC.m'];
ctIllTh = 300;
ctrxB = 50e6;
%% constants
LAMBDAMIN = 200;
...
%             Pcon = PCON*(100-rngDim(idIllSet))/100;
            ...
            for i=1:1:numel(A)
                C_LID(idLkD,idIllSet,idOpD) = C_LID(idLkD,idIllSet,idOpD) + log2(1 + ((Pj(i)*A(i))^2)/In(1));
            end
...
for idxOpD = 2:size(C_LID,3)
    ...
    plot(rngDim,C_LID(idLk2,:,idxOpD),plStyle);
...
if fSAVEALL
    fname = [ctDirRes 'fig_CapvsDimvsAp'];
    saveas(gcf,[fname '.png'],'png');
    saveas(gcf,[fname '.fig'],'fig');
    saveas(gcf,[fname '.eps'],'eps');
    fname = [ctDirRes 'CapvsDimvsAp'];
    saveas(gcf,[fname '.png'],'png');
    saveas(gcf,[fname '.fig'],'fig');
    saveas(gcf,[fname '.eps'],'eps');
    ifig = ifig + 1;
...
for idxOpD = 2:size(C_LID,3)
    ...
    iMK = rem(idxOpD,lenMK)+1;
    plStyle = [clLC{iLC} clLS{iLS} clMK{iMK}];
    plot(rngLkD,C_LID(:,idIll400,idxOpD),plStyle);
    lgd{idxOpD} = ['Do = ' num2str(rngOpD(idxOpD)*1e3,'%0.2f') 'mm'];
end
grid on;
...
% run('C:\\Users\pbutala\Documents\_Work\Code V2\Scratch\scratchSISO');
