...
switch fSTATION
    case 1
        ctDirRes = '\\ad\eng\users\p\b\pbutala\My Documents\MATLAB\Research\Code V2\Results\5. OSM-SVDmi\';
        ctFileCodeSrc = '\\ad\eng\users\p\b\pbutala\My Documents\MATLAB\Research\Code V2\Scripts\scrOSMimoSVD.m';
    case 2
        ctDirRes = '/home/pbutala/My Documents/MATLAB/Research/Code V2/Results/5. OSM-SVDmi/';
        ctFileCodeSrc = '/home/pbutala/My Documents/MATLAB/Research/Code V2/Scripts/scrOSMimoSVD.m';
    case 3
        ctDirRes = 'C:\\Users\\pbutala\\Documents\\MATLAB\\Research\\Code V2\\Results\\5. OSM-SVDmi\\';
...
% MONTE-CARLO RUNS
bit_err = zeros(numel(rngm),numel(rngmi));
Ill = zeros(numel(rngm),numel(rngmi));
figSNR = figure;
% set(gca,'YScale','log');
...
%         Syms = getPAMsyms(M);
        ...
        SymsPAM = mi*Syms*tP(1:Nt).' + repmat(tP(1:Nt).',M,1);
    ...
    % - BIT ERROR VS SNR
    semilogy(rngmi,bit_err(idm,:));                % plot bit error vs snr
...
legend(lgd);
if fSAVEALL
    fname = [ctDirRes flPreFix num2str(ifig) ' BER vs mi.fig'];
    saveas(gcf,fname,'fig');
    fname = [ctDirRes flPreFix num2str(ifig) ' BER vs mi.png'];
