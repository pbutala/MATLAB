...
switch fSTATION
    case 1
        ctDirRes = '\\ad\eng\users\p\b\pbutala\My Documents\MATLAB\Research\Code V2\Results\5. OSM-SVDsnr2\';
        ctFileCodeSrc = '\\ad\eng\users\p\b\pbutala\My Documents\MATLAB\Research\Code V2\Scripts\scrOSMimoSVDsnr.m';
    case 2
        ctDirRes = '/home/pbutala/My Documents/MATLAB/Research/Code V2/Results/5. OSM-SVDsnr2/';
        ctFileCodeSrc = '/home/pbutala/My Documents/MATLAB/Research/Code V2/Scripts/scrOSMimoSVDsnr.m';
    case 3
        ctDirRes = 'C:\\Users\\pbutala\\Documents\\MATLAB\\Research\\Code V2\\Results\\5. OSM-SVDsnr2\\';
        ctFileCodeSrc = 'C:\\Users\\pbutala\\Documents\\MATLAB\\Research\\Code V2\\Scripts\\scrOSMimoSVDsnr.m';
    otherwise
        error('Station not defined');
end
ctFileCodeDest = [ctDirRes 'scrOSMimoSVDsnr.m'];

% VARIABLES
% rnglkIl = 50:50:400;     % illumination level at center of room
% rnglkIl = 200;     % illumination level at center of room
lkIl = 400;
rngm = [1 2 4 8];  % range of #bits/symbol
% rngm = 2:2:4;
rngsnrdb = 5:5:50;
...
% MONTE-CARLO RUNS
bit_err = zeros(numel(rngm),numel(rngsnrdb));
Ill = zeros(numel(rngm),numel(rngsnrdb));
figSNR = figure;
set(gca,'YScale','log');
xlabel('SNR (dB)');
...
%         Syms = getPAMsyms(M);
        Syms = (-1:2/(M-1):1)'; % -1|00| -0.5|01| 0|erasure| 0.5|10| 1|11
        SymsPAM = Syms*tP(1:Nt).' + repmat(tP(1:Nt).',M,1);
        
        Pmt = zeros(Ntx,1);
        err = 0;                % Detected Errors
        iter = 1e6;            % # of Iterations
        for ii = 1:iter
            led_bits = randi([0,1],[1,log2(Nt)]);
            led_index = bin2dec(int2str(led_bits));   % transmitter index log2(M)+1:log2(Nt*M) (end)
            sym_bits = randi([0,1],[1,log2(M)]);
            sym_index = bin2dec(int2str(sym_bits));
            bit_stream = [sym_bits led_bits];
            ...
            pW = sqrt((((Hp*V*tx).^2)/snr)).*randn(NPx,1);
            ...
            for uu = 1:log2(Nt)
                led_bits_est(uu)= bin2dec(tmp1(uu)); % convert binary string to array of decimal 0/1 values.
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            % encoded in data.
            for xx = 1:log2(M)
                sym_bits_est(xx)= bin2dec(tmp2(xx));    % convert binary string to array of decimal 0/1 values.
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            
            rx_bits = [sym_bits_est led_bits_est];          % concatenate SYM bits and LED bits to recover frame.
            
            err = err + biterr2(rx_bits,bit_stream);
        end
    ...
    % - BIT ERROR VS SNR
    semilogy(rngsnrdb,bit_err(idm,:));                % plot bit error vs snr
